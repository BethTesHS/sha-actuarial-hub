...
export default function SuperAdminLogin({ theme = "dark" }) {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(async ({ data }) => {
      ...
      const profileRole = sessionUser ? await fetchProfileRole(sessionUser.id).catch(() => null) : null;
      if (sessionUser && isSuperAdminUser({ ...sessionUser, role: profileRole })) {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (!isSuperAdminUser({ ...signedInUser, role: profileRole })) {
